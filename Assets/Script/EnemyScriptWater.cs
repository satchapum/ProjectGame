...
public class EnemyScriptWater : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other) {
        if(other.CompareTag("FireBall") && gameObject.CompareTag("KnightWater")) 
        {
            GameObject obj = Instantiate(bullet, bulletPos.position, bulletPos.rotation);
            obj.SetActive(true);
        }
        else if(other.CompareTag("WaterBall") && gameObject.CompareTag("KnightWater")) 
        {
            health = health-((setting.allDamage)*0);
        }
        else if(other.CompareTag("WoodBall") && gameObject.CompareTag("KnightWater")) 
        {
